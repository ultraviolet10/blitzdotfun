...
import { type CreateContestInput, createContest, getAllContests } from "@/lib/contest"
...
async function handleCreateContest(request: NextRequest) {
    try {
        ...
        if (!body.name || !body.participantOne || !body.participantTwo || !body.contractAddress) {
            return NextResponse.json(
                {
                    error: "Missing required fields: name, participantOne, participantTwo, contractAddress",
                },
                { status: 400 },
            )
        }
        ...
        if (!body.participantOne.handle || !body.participantOne.walletAddress) {
            return NextResponse.json({ error: "participantOne must have handle and walletAddress" }, { status: 400 })
        }

        if (!body.participantTwo.handle || !body.participantTwo.walletAddress) {
            return NextResponse.json({ error: "participantTwo must have handle and walletAddress" }, { status: 400 })
        }
